...
namespace Debtor {
    ...
    internal class Program
    {
        ...
        static void Main()
        {
            ...
            var debtor_with_this_emails = debtors.Where(d => d.Email.EndsWith("@rhyta.com") || d.Email.EndsWith("@dayrep.com"));
            foreach (var debtor in debtor_with_this_emails)
            {
            ...
            var debtor_age_in_this_range = debtors.Where(d => d.BirthDay.Year >= 1987 && d.BirthDay.Year < 1997);
            foreach (var debtor in debtor_age_in_this_range)
            {
                Console.WriteLine(debtor);
            }
            Console.WriteLine("\n\n\n");
            ...
            var debtor_age_with_debt_over_5000 = debtors.Where(d => d.Debt <= 5000);
            foreach (var debtor in debtor_age_with_debt_over_5000)
            {
                Console.WriteLine(debtor);
            }
            Console.WriteLine("\n\n\n");
            ...
            var debtor_fullname_greater_than_18symbol_and_phone_number7_at_least_2 = debtors.Where(d => d.FullName.Length > 18 && d.Phone.Count(a => a == 7) >= 2);
            foreach (var debtor in debtor_fullname_greater_than_18symbol_and_phone_number7_at_least_2)
            {
                Console.WriteLine(debtor);
            }
            Console.WriteLine("\n\n\n");
            ...
            var debtor_which_birthdate_winter = debtors.Where(d => d.BirthDay.Month == 12 || d.BirthDay.Month == 1 || d.BirthDay.Month == 2);
            foreach (var debtor in debtor_which_birthdate_winter)
            {
                Console.WriteLine(debtor);
            }
            Console.WriteLine("\n\n\n");
            ...
            var debtor_with_debt_above_total_average_and_sort_firstly_surname_asc_next_debt_desc = debtors.Where(debtor => debtor.Debt > averageDebt).OrderBy(debtor => debtor.FullName).ThenByDescending(debtor => debtor.Debt);
            foreach (var debtor in debtor_with_debt_above_total_average_and_sort_firstly_surname_asc_next_debt_desc)
            {
            ...
            var debtor_phone_number_without_number8 = debtors.Where(d => !d.Phone.Contains("8"));
            foreach (var debtor in debtor_phone_number_without_number8)
            {
                Console.WriteLine($"{debtor.FullName}  {2023 - debtor.BirthDay.Year}  {debtor.Debt}");
            }
            Console.WriteLine("\n\n\n");
            ...
            var debtor_name_with_same_letter_at_least3 = debtors. Where(debtor => debtor.FullName.ToLower().GroupBy(a => a).Any(s => s.Count() >= 3)).OrderBy(debtor => debtor.FullName);
            foreach (var debtor in debtor_name_with_same_letter_at_least3)
            {
                Console.WriteLine(debtor);
            }
            Console.WriteLine("\n\n\n");
            ...
            int most_common_year = debtors.GroupBy(d => d.BirthDay.Year).OrderByDescending(year => year.Count()).Select(year => year.Key).FirstOrDefault();
            Console.WriteLine(most_common_year);
            Console.WriteLine("\n\n\n");

            //14)Borcu en boyuk olan 5 borclunun siyahisini cixartmaq

            var debtors_list_with_top5_debts = debtors.OrderByDescending(d => d.Debt).Take(5);
            foreach (var debtor in debtors_list_with_top5_debts)
            {
                Console.WriteLine(debtor);
            }
            Console.WriteLine("\n\n\n");
            ...
            int sum_of_all_debts = debtors.Sum(d => d.Debt);
            Console.WriteLine(sum_of_all_debts);
            Console.WriteLine("\n\n\n");

            //16)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq

            var debtors_lived_in_ww2 = debtors.Where(d => d.BirthDay.Year <= 1945);
            foreach (var debtor in debtors_lived_in_ww2)
            {
                Console.WriteLine(debtor);
            }
            Console.WriteLine("\n\n\n");
            ...
            var debtors_who_can_pay_debt_to_birtdate = debtors.Where(d => d.Debt <= 6000);
            foreach (var debtor in debtors_who_can_pay_debt_to_birtdate)
            {
                Console.WriteLine(debtor);
            }
            Console.WriteLine("\n\n\n");
            ...
            var debtors_who_have_smile_word_by_joint_letters = debtors.Where(d => d.FullName.Contains("s") && d.FullName.Contains("m") && d.FullName.Contains("i") && d.FullName.Contains("l") && d.FullName.Contains("e"));
            foreach (var debtor in debtors_who_have_smile_word_by_joint_letters)
            {
                Console.WriteLine(debtor);
            }
            Console.WriteLine("\n\n\n");
